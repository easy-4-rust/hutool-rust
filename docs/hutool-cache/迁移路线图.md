# hutool-cache Java → Rust 全量迁移路线图
...
|---|---|---|---|
| 生产对象 | `hutool-cache/src/main/java` | `crates/hutool-cache/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-cache/src/test`（7 个 Java 测试文件） | `crates/hutool-cache/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 19 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 19 | 25 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-cache/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 124 | 129 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 7 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-cache/src` |
| facade/mod/compat 内定义对象的文件 | — | 2 | 静态布局审计 |
| 已实现未验证 | — | 19 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.cache.impl.AbstractCache`（21 个公开/保护签名） | `crates/hutool-cache/src/impl/abstract_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.impl.NoCache`（20 个公开/保护签名） | `crates/hutool-cache/src/impl/no_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.CacheUtil`（10 个公开/保护签名） | `crates/hutool-cache/src/cache_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.file.AbstractFileCache`（10 个公开/保护签名） | `crates/hutool-cache/src/file_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.impl.CacheObj`（9 个公开/保护签名） | `crates/hutool-cache/src/impl/cache_obj.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.impl.ReentrantCache`（9 个公开/保护签名） | `crates/hutool-cache/src/impl/reentrant_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.impl.StampedCache`（7 个公开/保护签名） | `crates/hutool-cache/src/impl/stamped_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cache.file.LFUFileCache`（6 个公开/保护签名） | `crates/hutool-cache/src/file/lfu_file_cache.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-cache Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-cache Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
| Rust Cargo 直接依赖 | `moka, parking_lot, rand, tempfile` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-cache --all-targets --all-features
cargo test -p hutool-cache --all-targets --all-features
cargo fmt --all --check

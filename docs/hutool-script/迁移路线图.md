# hutool-script Java → Rust 全量迁移路线图
...
|---|---|---|---|
| 生产对象 | `hutool-script/src/main/java` | `crates/hutool-script/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-script/src/test`（2 个 Java 测试文件） | `crates/hutool-script/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 4 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 4 | 6 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-script/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 75 | 107 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 2 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-script/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 4 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.script.FullSupportScriptEngine`（22 个公开/保护签名） | `crates/hutool-script/src/full_support_script_engine.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.script.JavaScriptEngine`（22 个公开/保护签名） | `crates/hutool-script/src/full_support_script_engine.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.script.ScriptUtil`（18 个公开/保护签名） | `crates/hutool-script/src/script_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.script.ScriptRuntimeException`（13 个公开/保护签名） | `crates/hutool-script/src/script_runtime_exception.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-script Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-script Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `groovy-all, hutool-core, jython, luaj-jse` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `rhai, thiserror` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-script --all-targets --all-features
cargo test -p hutool-script --all-targets --all-features
cargo fmt --all --check

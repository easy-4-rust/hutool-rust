...
|---|---|---|---|
| 生产对象 | `hutool-json/src/main/java` | `crates/hutool-json/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-json/src/test`（122 个 Java 测试文件） | `crates/hutool-json/tests`（13 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 30 | 21 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-json/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 269 | 133 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 122 | 13 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-json/src` |
...
|---|---|---|---|
| `cn.hutool.json.JSONUtil`（58 个公开/保护签名） | `crates/hutool-json/src/facade/json_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.JSONArray`（55 个公开/保护签名） | `crates/hutool-json/src/compat.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.JSONObject`（36 个公开/保护签名） | `crates/hutool-json/src/compat.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.JSONConfig`（22 个公开/保护签名） | `crates/hutool-json/src/compat/json_config.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.JSONTokener`（20 个公开/保护签名） | `crates/hutool-json/src/parser/json_tokener.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.serialize.JSONWriter`（12 个公开/保护签名） | `crates/hutool-json/src/facade/json_writer.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.XML`（8 个公开/保护签名） | `crates/hutool-json/src/xml/xml.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.json.XMLTokener`（7 个公开/保护签名） | `crates/hutool-json/src/xml/xml_tokener.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-json Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-json Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
| Rust Cargo 直接依赖 | `base64, quick-xml, serde, serde_json, thiserror` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 10 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 10 个 | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-json --all-targets --all-features
cargo test -p hutool-json --all-targets --all-features
cargo fmt --all --check

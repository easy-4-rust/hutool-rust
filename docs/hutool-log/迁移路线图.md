...
|---|---|---|---|
| 生产对象 | `hutool-log/src/main/java` | `crates/hutool-log/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-log/src/test`（4 个 Java 测试文件） | `crates/hutool-log/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 34 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 34 | 26 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-log/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 251 | 43 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 4 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-log/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 34 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.log.StaticLog`（20 个公开/保护签名） | `crates/hutool-log/src/static_log.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.AbstractLog`（18 个公开/保护签名） | `crates/hutool-log/src/abstract_log.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.commons.ApacheCommonsLog`（17 个公开/保护签名） | `crates/hutool-log/src/dialect/commons.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.console.ConsoleLog`（16 个公开/保护签名） | `crates/hutool-log/src/dialect/console.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.jboss.JbossLog`（15 个公开/保护签名） | `crates/hutool-log/src/dialect/jboss.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.jdk.JdkLog`（15 个公开/保护签名） | `crates/hutool-log/src/dialect/jdk.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.log4j.Log4jLog`（15 个公开/保护签名） | `crates/hutool-log/src/dialect/log4j.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.log.dialect.log4j2.Log4j2Log`（15 个公开/保护签名） | `crates/hutool-log/src/dialect/log4j2.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-log Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-log Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `commons-logging, hutool-core, jboss-logging, log4j, log4j-api, log4j-core, logback-classic, logtube, slf4j-api, slf4j-simple, tinylog, tinylog-api, tinylog-impl` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `tracing, tracing-subscriber` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 0 个缺失、0 个骨架归零；布局审计归零 |
...
cargo check -p hutool-log --all-targets --all-features
cargo test -p hutool-log --all-targets --all-features
cargo fmt --all --check

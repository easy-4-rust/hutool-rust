...
|---|---|---|---|
| 生产对象 | `hutool-cron/src/main/java` | `crates/hutool-cron/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-cron/src/test`（18 个 Java 测试文件） | `crates/hutool-cron/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 32 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 34 | 33 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-cron/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 176 | 172 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 18 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-cron/src` |
| facade/mod/compat 内定义对象的文件 | — | 2 | 静态布局审计 |
| 已实现未验证 | — | 32 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.cron.Scheduler`（29 个公开/保护签名） | `crates/hutool-cron/src/scheduler.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.TaskTable`（17 个公开/保护签名） | `crates/hutool-cron/src/compat.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.CronUtil`（13 个公开/保护签名） | `crates/hutool-cron/src/cron_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.pattern.CronPattern`（9 个公开/保护签名） | `crates/hutool-cron/src/pattern/cron_pattern.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.timingwheel.TimerTaskList`（8 个公开/保护签名） | `crates/hutool-cron/src/timingwheel/timer_task_list.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.pattern.matcher.DayOfMonthMatcher`（7 个公开/保护签名） | `crates/hutool-cron/src/pattern/day_of_month_matcher.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.pattern.CronPatternUtil`（6 个公开/保护签名） | `crates/hutool-cron/src/pattern/cron_pattern_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.cron.pattern.matcher.BoolArrayMatcher`（6 个公开/保护签名） | `crates/hutool-cron/src/pattern/bool_array_matcher.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-cron Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-cron Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-core, hutool-log, hutool-setting, quartz` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `chrono, cron, thiserror, tokio, tracing` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 2 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 2 个 | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-cron --all-targets --all-features
cargo test -p hutool-cron --all-targets --all-features
cargo fmt --all --check

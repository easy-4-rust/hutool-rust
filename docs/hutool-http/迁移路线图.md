...
|---|---|---|---|
| 生产对象 | `hutool-http/src/main/java` | `crates/hutool-http/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-http/src/test`（37 个 Java 测试文件） | `crates/hutool-http/tests`（13 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 51 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 62 | 71 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-http/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 618 | 458 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 37 | 13 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-http/src` |
| facade/mod/compat 内定义对象的文件 | — | 2 | 静态布局审计 |
| 已实现未验证 | — | 51 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.http.HttpRequest`（80 个公开/保护签名） | `crates/hutool-http/src/request.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.HttpUtil`（55 个公开/保护签名） | `crates/hutool-http/src/http_util/http_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.HttpConnection`（35 个公开/保护签名） | `crates/hutool-http/src/http_connection/http_connection.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.webservice.JakartaSoapClient`（35 个公开/保护签名） | `crates/hutool-http/src/webservice/jakarta_soap_client.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.webservice.SoapClient`（35 个公开/保护签名） | `crates/hutool-http/src/webservice/soap_client.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.server.HttpServerRequest`（31 个公开/保护签名） | `crates/hutool-http/src/server/http_server_request.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.server.HttpServerResponse`（31 个公开/保护签名） | `crates/hutool-http/src/server/http_server_response.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.http.HttpResponse`（28 个公开/保护签名） | `crates/hutool-http/src/response/http_response.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-http Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-http Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-core, hutool-json, jakarta.xml.soap-api, javax.xml.soap-api` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `async-stream, encoding_rs, fancy-regex, flate2, http, hutool-core, indexmap, mime, rand, regex, reqwest, serde, serde_json, thiserror, tokio, url, woothee` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 11 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 11 个 | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-http --all-targets --all-features
cargo test -p hutool-http --all-targets --all-features
cargo fmt --all --check

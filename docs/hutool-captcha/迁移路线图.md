# hutool-captcha Java → Rust 全量迁移路线图
...
|---|---|---|---|
| 生产对象 | `hutool-captcha/src/main/java` | `crates/hutool-captcha/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-captcha/src/test`（5 个 Java 测试文件） | `crates/hutool-captcha/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 11 | 16 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-captcha/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 79 | 86 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 5 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-captcha/src` |
...
|---|---|---|---|
| `cn.hutool.captcha.AbstractCaptcha`（21 个公开/保护签名） | `crates/hutool-captcha/src/abstract_captcha.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.CaptchaUtil`（16 个公开/保护签名） | `crates/hutool-captcha/src/captcha_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.GifCaptcha`（11 个公开/保护签名） | `crates/hutool-captcha/src/gif_captcha.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.generator.MathGenerator`（7 个公开/保护签名） | `crates/hutool-captcha/src/generator/math_generator.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.CircleCaptcha`（6 个公开/保护签名） | `crates/hutool-captcha/src/circle_captcha.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.ShearCaptcha`（6 个公开/保护签名） | `crates/hutool-captcha/src/shear_captcha.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.LineCaptcha`（5 个公开/保护签名） | `crates/hutool-captcha/src/line_captcha.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.captcha.generator.RandomGenerator`（4 个公开/保护签名） | `crates/hutool-captcha/src/generator/random_generator.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-captcha Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-captcha Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-core` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `base64, font8x8, image, rand, thiserror` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-captcha --all-targets --all-features
cargo test -p hutool-captcha --all-targets --all-features
cargo fmt --all --check

# hutool-system Java → Rust 全量迁移路线图
...
|---|---|---|---|
| 生产对象 | `hutool-system/src/main/java` | `crates/hutool-system/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-system/src/test`（3 个 Java 测试文件） | `crates/hutool-system/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 14 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 14 | 23 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-system/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 176 | 78 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 3 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-system/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 14 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.system.OsInfo`（28 个公开/保护签名） | `crates/hutool-system/src/os_info.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.JavaInfo`（26 个公开/保护签名） | `crates/hutool-system/src/java_info.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.SystemUtil`（26 个公开/保护签名） | `crates/hutool-system/src/system_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.oshi.CpuInfo`（21 个公开/保护签名） | `crates/hutool-system/src/oshi/cpu_info.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.oshi.CpuTicks`（19 个公开/保护签名） | `crates/hutool-system/src/oshi/cpu_ticks.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.JavaRuntimeInfo`（14 个公开/保护签名） | `crates/hutool-system/src/java_runtime_info.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.oshi.OshiUtil`（11 个公开/保护签名） | `crates/hutool-system/src/oshi/oshi_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.system.UserInfo`（8 个公开/保护签名） | `crates/hutool-system/src/user_info.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-system Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-system Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-core, oshi-core, slf4j-simple` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `sysinfo` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-system --all-targets --all-features
cargo test -p hutool-system --all-targets --all-features
cargo fmt --all --check

...
|---|---|---|---|
| 生产对象 | `hutool-jwt/src/main/java` | `crates/hutool-jwt/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-jwt/src/test`（9 个 Java 测试文件） | `crates/hutool-jwt/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 15 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 15 | 24 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-jwt/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 112 | 196 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 9 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-jwt/src` |
| facade/mod/compat 内定义对象的文件 | — | 2 | 静态布局审计 |
| 已实现未验证 | — | 15 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.jwt.JWT`（30 个公开/保护签名） | `crates/hutool-jwt/src/jwt.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.signers.JWTSignerUtil`（23 个公开/保护签名） | `crates/hutool-jwt/src/signers/algorithm_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.JWTValidator`（8 个公开/保护签名） | `crates/hutool-jwt/src/jwt_validator.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.signers.AsymmetricJWTSigner`（8 个公开/保护签名） | `crates/hutool-jwt/src/signers/asymmetric_jwt_signer.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.JWTUtil`（7 个公开/保护签名） | `crates/hutool-jwt/src/jwt_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.Claims`（6 个公开/保护签名） | `crates/hutool-jwt/src/claims.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.JWTException`（6 个公开/保护签名） | `crates/hutool-jwt/src/jwt_exception.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.jwt.JWTHeader`（6 个公开/保护签名） | `crates/hutool-jwt/src/jwt_header.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-jwt Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-jwt Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `bcpkix-jdk18on, hutool-crypto, hutool-json, jjwt-gson, jjwt-impl` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `base64, jsonwebtoken, p256, p384, rsa, serde, serde_json, thiserror` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-jwt --all-targets --all-features
cargo test -p hutool-jwt --all-targets --all-features
cargo fmt --all --check

# hutool-crypto Java → Rust 全量迁移路线图
...
|---|---|---|---|
| 生产对象 | `hutool-crypto/src/main/java` | `crates/hutool-crypto/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-crypto/src/test`（32 个 Java 测试文件） | `crates/hutool-crypto/tests`（12 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 18 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 64 | 43 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-crypto/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 683 | 190 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 32 | 12 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-crypto/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 18 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.crypto.SecureUtil`（82 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.digest.DigestUtil`（53 个公开/保护签名） | `crates/hutool-crypto/src/hutool_facade/digest_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.KeyUtil`（52 个公开/保护签名） | `crates/hutool-crypto/src/key_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.asymmetric.SM2`（37 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.BCUtil`（27 个公开/保护签名） | `crates/hutool-crypto/src/bc_util/bc_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.asymmetric.Sign`（26 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.digest.Digester`（26 个公开/保护签名） | `crates/hutool-crypto/src/hutool_facade/digester.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.crypto.ECKeyUtil`（25 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-crypto Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-crypto Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `bcpkix-jdk18on, hutool-core` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `aes, aes-gcm, argon2, base64, block-padding, cbc, chacha20, cmac, data-encoding, der, des, ecb, generic-array, hex, hmac, md-5, num-bigint, p256, pbkdf2, rand, rand_core06, rc4, rsa, secrecy, sha1, sha2, signature, sm2, sm3, sm4, thiserror, x509-cert, zeroize` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 46 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 46 个 | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-crypto --all-targets --all-features
cargo test -p hutool-crypto --all-targets --all-features
cargo fmt --all --check

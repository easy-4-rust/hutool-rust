...
|---|---|---|---|
| 生产对象 | `hutool-aop/src/main/java` | `crates/hutool-aop/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-aop/src/test`（3 个 Java 测试文件） | `crates/hutool-aop/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 8 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 11 | 17 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-aop/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 26 | 41 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 3 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-aop/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 8 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.aop.proxy.ProxyFactory`（5 个公开/保护签名） | `crates/hutool-aop/src/proxy/proxy_factory.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.ProxyUtil`（4 个公开/保护签名） | `crates/hutool-aop/src/proxy/proxy_util.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.aspects.SimpleAspect`（3 个公开/保护签名） | `crates/hutool-aop/src/aspects/simple_aspect.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.interceptor.CglibInterceptor`（3 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.interceptor.JdkInterceptor`（3 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.interceptor.SpringCglibInterceptor`（3 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.aspects.TimeIntervalAspect`（2 个公开/保护签名） | `crates/hutool-aop/src/aspects/time_interval_aspect.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.aop.proxy.CglibProxyFactory`（1 个公开/保护签名） | `crates/hutool-aop/src/proxy/cglib_proxy_factory.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-aop Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-aop Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `cglib, hutool-core, spring-core` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `parking_lot` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS` | 3 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 3 个 | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-aop --all-targets --all-features
cargo test -p hutool-aop --all-targets --all-features
cargo fmt --all --check

...
|---|---|---|---|
| 生产对象 | `hutool-ai/src/main/java` | `crates/hutool-ai/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-ai/src/test`（10 个 Java 测试文件） | `crates/hutool-ai/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 48 | 24 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-ai/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 215 | 73 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 10 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0（2026-08-04 清零） | `rg -l 'todo!\|unimplemented!' crates/hutool-ai/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 46（2026-08-04：+6 厂商 Provider 30 对象 + openai 承载） | 有真实代码与 Rust 本地测试，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.ai.core.BaseConfig`（17 个公开/保护签名） | `crates/hutool-ai/src/core/base_config.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.doubao.DoubaoServiceImpl`（17 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.gemini.GeminiServiceImpl`（17 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.grok.GrokServiceImpl`（14 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.openai.OpenaiServiceImpl`（14 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.ollama.OllamaServiceImpl`（13 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.model.hutool.HutoolServiceImpl`（12 个公开/保护签名） | `—` | `NOT_STARTED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.ai.core.AIConfigBuilder`（11 个公开/保护签名） | `crates/hutool-ai/src/core/ai_config_builder.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-ai Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-ai Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-core, hutool-http, hutool-json, hutool-log` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `async-stream, async-trait, futures-core, hutool-http, reqwest, secrecy, serde, serde_json, thiserror, tokio, url` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
| S1 | 一对象一文件与签名补齐 | `IN_PROGRESS`（2026-08-04：6 厂商 provider 已实现并注册，缺失 34 → 2） | 0 个缺失、0 个骨架归零；布局审计归零 |
...
| P0 | 补齐无 Rust 对应对象 | 34 个（2026-08-04 已完成 6 厂商 30 对象：Provider 注册 + 端点承载） | 对象、方法、参数与来源注释完整 |
...
cargo check -p hutool-ai --all-targets --all-features
cargo test -p hutool-ai --all-targets --all-features
cargo fmt --all --check

...
|---|---|---|---|
| 生产对象 | `hutool-dfa/src/main/java` | `crates/hutool-dfa/src` | 每个对象、方法、重载、参数有去向 |
| 测试与夹具 | `hutool-dfa/src/test`（2 个 Java 测试文件） | `crates/hutool-dfa/tests`（1 个 Rust 集成测试文件） | SOURCE_PARITY 三本账 + E3/E4 差分 |
| Rust 已实现内容 | Java 对象分母不变 | 当前 5 个真实逻辑候选 | 不删除、不降级，补证据或增量修正 |
...
| 维度 | Java | Rust | 证据 |
|---|---:|---:|---|
| 生产对象主文件 | 5 | 10 | `rg --files .../src/main/java -g '*.java'`；`rg --files crates/hutool-dfa/src -g '*.rs'` |
| 公共/保护方法与构造器（静态解析） | 38 | 47 个 `pub fn` 候选 | 本表生成器正则清单；需语言级工具复核 |
| 测试文件 | 2 | 1 | 固定 SHA 工作树扫描 |
| 占位 Rust 文件 | — | 0 | `rg -l 'todo!\|unimplemented!' crates/hutool-dfa/src` |
| facade/mod/compat 内定义对象的文件 | — | 1 | 静态布局审计 |
| 已实现未验证 | — | 5 | 有真实代码，无双端差分 |
...
|---|---|---|---|
| `cn.hutool.dfa.SensitiveUtil`（17 个公开/保护签名） | `crates/hutool-dfa/src/sensitive.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.dfa.WordTree`（15 个公开/保护签名） | `crates/hutool-dfa/src/word_tree/word_tree.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.dfa.FoundWord`（4 个公开/保护签名） | `crates/hutool-dfa/src/word_tree/found_word.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.dfa.StopChar`（2 个公开/保护签名） | `crates/hutool-dfa/src/stop_char.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |
| `cn.hutool.dfa.SensitiveProcessor`（0 个公开/保护签名） | `crates/hutool-dfa/src/sensitive_processor/sensitive_processor.rs` | `IMPLEMENTED_UNVERIFIED` | 调用者、错误路径、副作用、源测试处置、E3/E4 差分 |

```mermaid
flowchart LR
    J["hutool-dfa Java 公共入口"] --> JC["协作者/工厂/策略/资源"]
    R["hutool-dfa Rust 公共入口"] --> RC["trait/struct/registry/async"]
    JC --> JE["JVM 与外部组件边界"]
...
|---|---|---|---|
| Java POM 直接 artifact | `hutool-json` | `SOURCE_DECLARED` | 从 Java 调用链提取真实合同 |
| Rust Cargo 直接依赖 | `aho-corasick, parking_lot, serde, serde_json, thiserror` | `DEPENDENCY_DECLARED` | license/MSRV/target/advisory + 风险 POC + 共享合同测试 |
...
cargo check -p hutool-dfa --all-targets --all-features
cargo test -p hutool-dfa --all-targets --all-features
cargo fmt --all --check

//! `CircleCaptcha` 对象的独立模块。
//!
//! 对齐: `cn.hutool.captcha.CircleCaptcha`
//! 来源: `hutool-captcha/src/main/java/cn/hutool/captcha/CircleCaptcha.java`
//! 中文说明: 圆圈干扰验证码类型，封装圆形干扰场景下的构造入口与即时渲染能力。
...
/// Hutool 兼容的圆圈干扰验证码。
///
/// 对齐 Java `CircleCaptcha`：默认 5 位验证码和 15 个干扰圆圈。
#[derive(Debug)]
pub struct CircleCaptcha(pub(crate) AbstractCaptcha);

impl CircleCaptcha {
    /// 创建默认配置的圆圈干扰验证码。
    pub fn new(width: u16, height: u16) -> Result<Self, CaptchaError> {
        Self::with_code_count(width, height, 5, 15)
    }
...
impl DerefMut for CircleCaptcha {

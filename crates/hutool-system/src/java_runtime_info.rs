...
mod tests {
    ...
    fn default_arrays_are_empty() {
        let runtime = JavaRuntimeInfo::default();
        assert!(runtime.class_path_array().is_empty());
        assert!(runtime.library_path_array().is_empty());
    }

    #[test]
    fn class_and_library_paths_split_by_host_separator() {
        let separator = PATH_SEPARATOR_TEST;
    ...
    #[test]
    fn split_paths_handles_none() {
        assert_eq!(split_paths(None), Vec::<PathBuf>::new());
    }

    #[test]
    fn detect_keeps_optional_fields() {
        let runtime = JavaRuntimeInfo::detect();
        // 验证结构/Debug 输出正常
        assert!(format!("{runtime:?}").contains("JavaRuntimeInfo"));
    }
}

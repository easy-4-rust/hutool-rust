...
//! 中文说明: 字符串定义，Hutool `text::StrUtil` 的 idiomatic Rust 实现（自由函数 + Result 错误处理）。
//! 自 `src/string.rs`（932 行）拆分而来，扩展 trait 见 [`crate::text::str_ext`]。
...
///
/// 已迁移至 [`crate::text::str_ext::StrExt`]，此处仅保留 `pub use` 重导出，
/// 保持公开 API 完全兼容。
pub use crate::text::str_ext::StrExt;

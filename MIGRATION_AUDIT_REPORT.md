...
| 违规类型 | 文件数 | 状态 |
|---------|--------|------|
| 普通 .rs 文件含 2+ pub 类型 | 72 | ⚠️ 待修复 |
| 其中含 macro_rules! 的文件 | 13 | ⚠️ 需手动拆分 |
| 其中无 macro 的文件 | 59 | ⚠️ 脚本构建失败 |
| lib.rs 含多类型定义 | 0 | ✅ 合规 |
| **总计违规** | **72** | |

**已成功拆分**: 100 个文件 (从 172 减少到 72)
...
剩余工作：
1. 拆分 72 个多类型文件 (13 个 macro 文件需手动拆分，59 个需修复跨模块私有字段访问)
...
- **私有字段跨模块访问**: 拆分后 struct 字段变为跨模块不可见
- **impl 块归属**: 多个 impl 块可能被错误分配到不同 type 文件
